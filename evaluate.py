...
def evaluateRandomly(encoder, decoder, voc, pairs, reverse, beam_size, n=10):
    ...
    for _ in range(n):
        ...
        if beam_size == 1:
            ...
            rouge_1 = rouge.rouge_n(summary=output_sentence,references=pair[1],n=1)
            r1 += rouge_1
            rouge_2 = rouge.rouge_n(summary=output_sentence,references=pair[1],n=2)
            r2 += rouge_2
            rouge_l = rouge.rouge_l(summary=output_sentence,references=pair[1])
            rl += rouge_l
            # You need spaCy to calculate ROUGE-BE

            rouge_be = rouge.rouge_be(summary=output_sentence,references=pair[1])
            rbe += rouge_be
            print("ROUGE-1: {}, ROUGE-2: {}, ROUGE-L: {}, ROUGE-BE: {}".format(rouge_1, rouge_2, rouge_l, rouge_be).replace(", ", "\n"))
            bleu_score = bleu.bleu(output_sentence,pair[1])
            bleutotal += bleu_score
    ...
    print("=============================================================")
    print("AVG ROUGE-1: {}\nAVG ROUGE-2: {}\nAVG ROUGE-L: {}\nAVG ROUGE-BE: {}\n".format(r1/100, r2/100, rl/100, rbe/100).replace(", ", "\n"))
    print("AVG bleu: %f"%(bleutotal/100))
...
def runTest(n_layers, hidden_size, reverse, modelFile, beam_size, inp, corpus):
    ...
    if inp:
        ...
        evaluateRandomly(encoder, decoder, voc, pairs, reverse, beam_size, 100)
